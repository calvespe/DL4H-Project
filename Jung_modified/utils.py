...
def normalize_feature(train_data):
    ...
    demograhpic_features = train_data[:, :, 0:3]
    genetic_features = train_data[:, :, 15].reshape(-1, sequence_length, 1)
    ...
    # concact
    t_tmp = np.concatenate((t_tmp, demograhpic_features), axis=2)
    t_tmp = np.concatenate((t_tmp, genetic_features), axis=2)

    # generate mask
    #print(t_tmp.shape)
    N, timesteps, n_features = t_tmp.shape
    mask = np.ones_like(t_tmp.reshape(-1,n_features))
    mask[np.where(t_tmp.reshape(-1, n_features) == 0)] = 0
    #print('mask', mask.shape)
    
    return t_tmp.astype(float), mask.reshape(-1,sequence_length ,n_features).astype(float)
