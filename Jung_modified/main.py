...
for k in range(args.kfold-1,-1,-1):
    ...
    else: # default params go to here
    ...
    if args.cognitive_score == True:
        ...
        print('norm_train_feature', norm_train_feature.shape)
        print('train_cog_norm_feature', train_cog_norm_feature.shape)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase != 'train':
                        if args.task == 1:  # single-task
                            ...
                            plot_mae_cog1, plot_mse_cog1 = regression_cog(f, ret['predict_mmse'].data.cpu().numpy(), data['forward']['values'][:, 1:, 10:11].data.cpu().numpy(), data['forward']['masks'][:, 1:, 10:11].data.cpu().numpy(), original_cog=np.array([30]))
                            plot_mae_cog2, plot_mse_cog2 = regression_cog(f, ret['predict_ad11'].data.cpu().numpy(), data['forward']['values'][:, 1:, 11:12].data.cpu().numpy(), data['forward']['masks'][:, 1:, 11:12].data.cpu().numpy(), original_cog=np.array([70]))
                            plot_mae_cog3, plot_mse_cog3 = regression_cog(f, ret['predict_ad13'].data.cpu().numpy(), data['forward']['values'][:, 1:, 12:13].data.cpu().numpy(), data['forward']['masks'][:, 1:, 12:13].data.cpu().numpy(), original_cog=np.array([85]))
