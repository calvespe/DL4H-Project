...
class Cog_Model(nn.Module):
    def __init__(self, rnn_hid_size, impute_weight, reg_weight, label_weight, classes=False):
        ...
        self.features = 13
        self.cog_index = self.features - 3
        self.seq_length = 10
        self.build()
    ...
    def forward(self, data, direct, criterion_reg, criterion_cls, multi_flag=False):
        ...
        shifted_data = data[direct]['values'][:, 1:, :self.cog_index]
        shifted_mask = data[direct]['masks'][:, 1:, :self.cog_index]

        if multi_flag == True:
            output_cls = torch.cat(output_cls,  dim = 1)
            output_probs = torch.cat(output_probs, dim = 1)

        #print('output_reg', output_reg.shape)
        #print('shifted_data', shifted_data.shape)
        y_reg_loss = criterion_reg(output_reg.contiguous().view(-1, self.cog_index) * shifted_mask[:,:,:6].contiguous().view(-1, self.cog_index),
                                   (shifted_data[:,:,:6] * shifted_mask[:,:,:6]).contiguous().view(-1, self.cog_index))
        #raise NotImplementedError
        y_mmse_loss = criterion_reg(output_mmse.contiguous()*data[direct]['masks'][:, 1:, self.cog_index:self.cog_index+1],
                                    (data[direct]['values'][:, 1:, self.cog_index:self.cog_index+1]*data[direct]['masks'][:, 1:, self.cog_index:self.cog_index+1]))
        y_ad11_loss = criterion_reg(output_ad11.contiguous()* data[direct]['masks'][:, 1:, self.cog_index+1:self.cog_index+2],
                                    (data[direct]['values'][:, 1:, self.cog_index+1:self.cog_index+2] * data[direct]['masks'][:, 1:, self.cog_index+1:self.cog_index+2]))
        y_ad13_loss = criterion_reg(output_ad13.contiguous()* data[direct]['masks'][:, 1:, self.cog_index+2:self.cog_index+3],
                                    (data[direct]['values'][:, 1:, self.cog_index+2:self.cog_index+3] * data[direct]['masks'][:, 1:, self.cog_index+2:self.cog_index+3]))
        if multi_flag == True:
