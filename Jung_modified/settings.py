...
def parse_delta(masks, dir_):
    ...
    deltas = []
    timesteps, n_features = masks.shape

    for h in range(11):
        if h == 0:
            # deltas.append(np.zeros(6)*12)
            deltas.append(np.zeros(n_features) * 12)
        else:
            # deltas.append((np.ones(6)*12) + ((1 - masks[h]) * deltas[-1]))
            deltas.append((np.ones(n_features) * 12) + ((1 - masks[h]) * deltas[-1]))
...
def sample_loader(data, mask, label, batch_size, is_train=False):
    ...
    N, timesteps, n_features = data.shape
    mri_data_index = n_features-3
    mri_data = data[:,:,:mri_data_index]
    cog_data = data[:,:,mri_data_index:]

    mri_mask = mask[:,:,:mri_data_index]
    cog_mask = mask[:,:,mri_data_index:]
    #print('mri_data', mri_data.shape)
    #print('cog_data', cog_data.shape)
...
def regression(f, imputations, evals, eval_masks, c, m):
    ...
    evals = evals[:,:,:6]
    eval_masks = eval_masks[:,:,:6]

    """ print('imputations', imputations.shape)
